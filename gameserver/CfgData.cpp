...
bool CfgData::init(int32_t equipIdInterval, int32_t viceGeneralIdInterval,int32_t lackeyInterval, int32_t debug)
{
	...
	InitOnLimeReward();					// 初始化在线时间
	InitSevenLoginReward();				// 初始化七天登陆奖励
	InitLevelGift();					// 初始化等级礼包
	InitWeekOnlineReward();				// 初始化周在线奖励
	InitOffLineExpTable();				// 初始化离线奖励表
	InitVipCardTable();					// 初始化vipcard
	InitBossHomeTable();				// 初始化boss之家
	InitVipGuaJiMap();					// 初始化vip挂机地图
	InitFunctionOpenTable();			// 初始化功能开启
	InitMoLingRuQinMap();				// 初始化魔灵入侵地图
	InitAutoPetGfit();					// 初始化自动使用的宠物礼包
	InitPlantEventTable();				// 初始化采集物事件表
	InitZiYuanZhaoHuiTable();			// 初始化资源找回表
	InitMoHuaHuanYiTable();				// 初始化魔化转移表
	InitShouChongLiBao();				// 初始化首冲礼包表
	InitNewServerFavorable();			// 新服特惠礼包
	InitEveryDayChongZhi();				// 初始化每日充值
	InitKaiFuHuoDongTable();			// 初始开服活动
	InitHuoYueDuTable();				// 初始化活跃度表
	InitGuangWeiTable();				// 初始化官位表
	InitAppendAttrTable();				// 初始化附加属性表
	InitwuHunExpTable();				// 初始化武魂经验表
	InitPetBuyTable();					// 初始化购买宠物幻化
	InitFamilyRewardTable();			// 初始化军团奖励表
	InitAchievementTable();				// 初始化成就表
	InitXunZhangTable();				// 初始化勋章表
	InitBuyAcSocreTable();				// 初始化购买成就积分
	InitTotalChongZhiTable();			// 初始化开服累计充值
	InitHuanHuaNeedRoleLevelTable();	// 初始化幻化等级需求表
	InitWarVictoryTable();				// 初始化抗战胜利奖励表

	m_TouZiCfg.InitTouZiTable();		// 初始化投资配置
	m_ChouJiangConfig.InitCJConfig();	// 初始化抽奖配置
	m_QiFuTable.InitQiFuTable();		// 初始化祈福配置
	m_VipTable.InitVipTable();			// 初始化vip配置
	m_debug = debug;
...
void CfgData::InitGuangWeiTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_GUAN_WEI_TABLE");
		return;
	}
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_QI_SHI_TABLE");
		return;
	}
...
void CfgData::InitBuyAcSocreTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_BUY_AC_SOCRE_TABLE");
		return;
	}
...
void CfgData::InitXunZhangTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_XUN_ZHANG_TABLE");
		return;
	}
...
void CfgData::InitAchievementTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_ACHIEVEMENT_TASK_TABLE");
		return;
	}
...
void CfgData::InitFamilyRewardTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_FAMILY_TASK_RING");
		return;
	}
...
void CfgData::InitPetBuyTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_BUY_PET_INFO");
		return;
	}
...
void CfgData::InitwuHunExpTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_WU_HUN_EXP");
		return;
	}
...
void CfgData::InitAppendAttrTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_APPEND_ATTR_TABLE");
		return;
	}
...
void CfgData::InitHuoYueDuTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_HUO_YUE_DU_TABLE");
		return;
	}
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_HUO_YUE_DU_REWARD_TABLE");
		return;
	}
...
void CfgData::InitKaiFuHuoDongTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误FILE_KAI_FU_HUO_DONG");
		return;
	}
...
void CfgData::InitHuanHuaNeedRoleLevelTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_HUAN_HUA_NEED_ROLE_LEVEL_TABLE");
		return;
	}
...
void CfgData::InitWarVictoryTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_WAR_VICTORY_TABLE");
		return;
	}
...
void CfgData::InitTotalChongZhiTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_TOTAL_CHONG_ZHI_TABLE");
		return;
	}
	...
	for( int32_t i = 0;i < iBaseTableCount -1 ;i++ ) //这张表最后一样服务端没用
	{
...
void CfgData::InitEveryDayChongZhi()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_EVERYDAY_CHONG_ZHI");
		return;
	}
...
void CfgData::InitNewServerFavorable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_NEW_SERVER_FAVORABLE");
		return;
	}
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_THREE_PET_GIFT");
		return;
	}
...
void CfgData::InitShouChongLiBao()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_SHOU_CHONG_LI_BAO");
		return;
	}
...
void CfgData::InitMoHuaHuanYiTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_MO_HUA_ZHUAN_YI_TABLE");
		return;
	}
...
void CfgData::InitMoLingRuQinMap()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_MO_LING_RU_QING_HD_TABLE");
		return;
	}
...
void CfgData::InitFunctionOpenTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_FUNCTION_OPEN_TABLE");
		return;
	}
...
void CfgData::InitVipGuaJiMap()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_VIP_GUA_JI_MAP");
		return;
	}
...
void CfgData::InitBossHomeTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_BOSS_HOME_TABLE");
		return;
	}
...
void CfgData::InitVipCardTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_VIP_CARD_TABLE");
		return;
	}
...
void CfgData::InitOffLineExpTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_OFF_LINE_EXP_TABLE");
		return;
	}
...
void CfgData::InitWeekOnlineReward()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_WEEK_ONLINE_REWARD");
		return;
	}
...
void CfgData::InitLevelGift()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_LEVEL_GIFT");
		return;
	}
...
void CfgData::InitSevenLoginReward()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_SEVEN_LOGIN_REWARD");
		return;
	}
...
void CfgData::InitOnLimeReward()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_ONLINE_REWARD");
		return;
	}
...
void CfgData::fetchSignReward()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("打开配置表错误 FILE_SIGIN_REWARD_TABLE");
		return;
	}
...
void CfgData::InitWuHunHoleTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("open FILE_ITEM_WUHUN_HOLE_TABLE 失败,请检查文件名大小写");
		return;
	}
...
void CfgData::InitWuHunMoHunTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("open FILE_ITEM_WUHUN_MOHUN_TABLE 失败,请检查文件名大小写");
		return;
	}
...
void CfgData::InitPlayerInitPetTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("open FILE_PLAYER_INIT_PET_TABLE 失败,请检查文件名大小写");
		return;
	}
...
void CfgData::InitPetPackageTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("open FILE_PET_PACKAGE_TABLE 失败,请检查文件名大小写");
		return;
	}
...
void CfgData::InitPetEggTable()
{
	...
	if ( ret == false )
	{
		LOG_ERROR("open FILE_PET_EGG_TABLE 失败,请检查文件名大小写");
		return;
	}
