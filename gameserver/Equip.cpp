...
#define USUAL_ITEM		1  //使用普通物品
#define SPECIAL_ITEM	2  //使用特殊物品

#define SPECIAL_STAT	9					//特殊的星级,9级以后不能用普通材料
#define NEED_GONG_GAO_STAT 6				//6星要公告
#define XIANG_QIAN_GONG_GAO_GEM_LEVEL	5	//宝石的最大等级
int32_t GemOpenNeedStarLevel[GEM_SLOT_NUM] = { 4, 5, 6 };
#define MAX_SUIT_EQUIP	 120
enum ERR_EQUIP
{
	ERR_EQUIP_UPGRADE_FAIL		= 1,	// 装备升阶失败
	ERR_EQUIP_UPQUALITY_FAIL	= 2,	// 装备升品失败
	ERR_EQUIP_UPSTAR_FAIL		= 3,	// 装备升星失败
};
...
void CExtEquip::OnCleanUp()
{
	...
	m_lstDirty.clear();										// 脏数据

	m_nEquipGoalStarLevel	= 0;							// 套装强化等级
	m_nEquipGoalGemLevel	= 0;							// 套装宝石等级
	m_mEquipSuit.clear();									// 套装数量
}
