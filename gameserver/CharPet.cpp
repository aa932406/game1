...
{
	if ( dbData.m_CharPets.charPets.empty() )	// 初始化没有宠物，送两只
	{
		if ( NULL == m_pPlayer )
		{
			return;
...
int32_t CExtCharPet::onPetRelease( Answer::NetPacket* inPacket )			// 放生宠物
{
	if ( NULL == m_pPlayer || NULL == inPacket )
	{
		return ERR_INVALID_DATA;
...
int32_t CExtCharPet::onUseExpItem( Answer::NetPacket* inPacket )
{
	...
	PET_RECORD record;
	int32_t nAddExp = 0;
	if ( slot.itemId == ISI_PET_EXP_GRAIN )				// 圣兽魔晶
	{
		record = PR_USE_EXP_GRAIN;
		bNeedCheck = true;
		nAddExp = atoi( cfgItem->effect.c_str() );
	}
	else if ( slot.itemId == ISI_EXP_BALL )				// 经验球
	{
	...
	else if ( slot.itemId == ISI_SPECIAL_EXP_BALL )		// 特级经验球
	{
		if( pPet->getLevel() >= m_pPlayer->getLevel() + LEVEL_DIFF )
		{
			return ERR_INVALID_DATA;
...
int32_t CExtCharPet::onUsePhaseItem( Answer::NetPacket* inPacket )
{
	...
	if ( slot.itemId == ISI_PET_KIWI_FRUIT )				// 奇异果
	{
		phase = PP_INVALID;	// 自动随机
	}
	else if ( slot.itemId == ISI_PET_ELECTRIC_POTION )		// 电浆药水
	{
		phase = PP_ELECTRIC;
	}
	else
...
int32_t CExtCharPet::onIllusion( Answer::NetPacket* inPacket )
{
	...
	int8_t	nType		= inPacket->readInt8();		// 基础属性幻化、成长率幻化、幸运值幻化
	int8_t	nUseItem	= inPacket->readInt8();		// 成长率幻化：0 不使用道具 1 幸运草 2 神仙草
