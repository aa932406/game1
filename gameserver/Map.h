...
public:
	virtual void init( const CfgMap &cfgmap );//地图数据初始化
	virtual void update();
	
	virtual int32_t canEnter(Player *player);//判断是否可以进入
...
public:
	...
	void sendTeamsInMap( Player *player );			// 查找附近队伍时下发
