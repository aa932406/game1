...
public:
	...
	int32_t getActivityBirthRegion(Int32Vector* pRegsions) const;//取得国家的出生点
	int32_t leaveDungeon();
	bool isInRectangle(Position x,Position y) const;
	//bool inMatch() const;//在比武中
	bool isLeader() const; //是否是帮主
	...
	void addExp(int64_t addon , bool Iswallow = true ); //Iswallow 是否受防沉迷系统影响 
	int64_t GetLevelExp();
	...
	double benefitRatio();
	BenefitType benefitType();//收益类型。损害。疲劳等
	...
	//void  addJungong(int32_t addon,int32_t reason, BenefitType bnfType);//阅历
	...
	int32_t verifyBagInfo( const Int32Vector& vSlot,int32_t nItemId,int32_t nCount);//核实包信息
	int32_t queryAutoBuyBagInfo( const Int32Vector& vSlot,int32_t nItemId,int32_t& count,int32_t nShopId);//自动购买核实包信息
	...
	void onDamageEvent(UnitHandle target, int32_t attackValue);									// 伤害消息
	void onDamagedEvent(UnitHandle launcher, int32_t attackValue);								// 伤害消息
...
//	int32_t onQueryMonsterBroadcast(Answer::NetPacket *inPacket);
	...
	void doSkillCost(CfgSkill *pCfgSkill, MemChrSkillVector::iterator &itSkill);				// 技能消耗
...
private:
	...
	PlayerPosition m_ActFrontPosition; //进入活动前的位置
	...
	int32_t m_todayGoldCharge;//今日充值元宝
	int32_t m_firstFiveDayGoldCharge;//开服10天充值元宝
...
//复活保护
	...
	int32_t				m_StartProtect;								//开始保护的时间
	int32_t				GetProtectTime();							//获取保护时间
	void				DieResetXp();
	void				SendHDIcon();
	int32_t				OnRandPos( int32_t BagSlot );				//使用随机卷轴
	int32_t				OnBackCity( int32_t BagSlot );				//使用回城卷
	void				PayedDispose( int32_t AddGold );			//充值以后的处理
