...
#define BATTLE_TIAN_FU  126			//战斗力天赋技能id
#define REPLY_HP_CD     10 * 1000	//回血cd
#define REPLAY_HP_CD_90 2 * 1000	//30级前回血cd
...
bool CObjPet::AppendInfo(Answer::NetPacket *packet)
{
	...
	packet->writeInt64( GetPetId() );					// 宠物ID
	packet->writeInt32( GetBaseId() );					// 配置ID
	packet->writeUTF8( GetName() );						// 宠物名称
	packet->writeInt64( m_pPlayer->getCid() );			// 拥有者ID
	packet->writeUTF8( m_pPlayer->getName() );			// 拥有者名称
	packet->writeInt16( getCurrentTile().x );			// 当前坐标X
	packet->writeInt16( getCurrentTile().y );			// 当前坐标Y
	packet->writeInt16( getTargetTile().x );			// 目标坐标X
	packet->writeInt16( getTargetTile().y );			// 目标坐标Y
	packet->writeInt32( GetHP() );						// 当前血量
	packet->writeInt32( GetMaxHP() );					// 最大血量
	packet->writeInt32( GetMoveSpeed() );				// 移动速度
	packet->writeInt32( GetTopRankIndex() );			// 最大名次（圣兽）
	packet->writeInt32( GetPoints() );					// 品质
...
void CObjPet::SendPetStatus()
{
	...
	packet->writeInt64( m_pPet->GetPetId() );		// 宠物ID
	packet->writeInt32( m_pPet->GetPoints() );		// 评分
	packet->writeInt32( GetTopRankIndex() );		// 宠物排名
...
void CObjPet::AppendPetInfo( Answer::NetPacket* packet )
{
	...
	packet->writeInt8( m_nBagType );							// 存储位置
	packet->writeInt64( m_pPet->GetPetId() );					// 宠物ID
	packet->writeInt32( m_pPet->GetFlag() );					// 更新标记
	packet->writeInt8( m_bAlive ? 1 : 0 );						// 是否存活
	packet->writeInt32( m_nSoul );								// 死亡魂魄值
	packet->writeInt8( static_cast<int8_t>( m_nState ) );		// 宠物当前状态
	packet->writeInt8( m_nKnight );								// 骑士
	packet->writeInt8( m_nQiShi );								// 佩戴的坐骑
	appendAttrInfo( packet );									// 宠物属性
}
...
void CObjPet::appendAttrInfo( Answer::NetPacket* packet )
{
	...
	packet->writeInt32( GetAttrValue( CObjAttrs::ATTR_CRITRATE ) );		// 暴击（预留）
	packet->writeInt32( GetAttrValue( CObjAttrs::ATTR_TENACITY ) );		// 韧性（预留）
}
...
int32_t CObjPet::calBaseAttr( PET_ATTR nStart, PET_ATTR nGrow, int32_t nLevel )
{
	...
	return nStartAttr + nGrowAttr * ( nLevel - 1 ) / 1000;	// 表中数值是实际数值的1000倍
}
...
void CObjPet::addPassiveSkillAttr()
{
	...
	for ( int32_t i = 1; i < PET_SKILL_BAR_SIZE; ++i )
	{
		...
		if ( pSkillInfo->nId  == BATTLE_TIAN_FU )	//战斗力不在这里加
		{
			continue;
		}
