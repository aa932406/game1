...
void GameService::onNewMinuteCome(int32_t minute)
{
	...
	{
		DB_SERVICE.onNewMinuteCome();		//更新到DBServer
...
{
	// 此时角色还未添加进地图中，还在网络线程处理,不需要进行线路检测
	...
	for ( ; iter != eiter; ++iter )
	{
		...
		if ( it != m_players.end() )
		{
			...
			if ( pPlayer != NULL )
			{
				...
				GAME_SERVICE.replySuccess( pPlayer->getGateIndex(), CM_ENTER_DUNGEON, pDungeon->getDungeonId() );	// 需要早于副本信息下发给客户端
