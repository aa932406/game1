...
FindNode	mapnods[TPOC_MAP_RUNNER_SIZE][MAP_MAX_NOD][MAP_MAX_NOD];
...
PositionVector PathFinder::findPath(Position src, Position dest)
{
	...
	if (pSrc != NULL && pDest != NULL && pDest->isWalkable() && !m_map.inStack( dest.x, dest.y ))
	{
...
{
	bool bFind = false;
	int32_t nRunnerId = m_map.GetRunnerId();
	int32_t nIdx = nRunnerId - 1;
	PositionVector path;
	if ( nIdx < 0 || nIdx >= TPOC_MAP_RUNNER_SIZE )
	{
		return path;
	}
	bzero( mapnods[nIdx], sizeof( FindNode )* MAP_MAX_NOD * MAP_MAX_NOD );
	...
	if (pSrc != NULL && pDest != NULL && pDest->isWalkable() && !m_map.inStack( dest.x, dest.y ) )
	{
		...
		mapnods[nIdx][src.x][src.y].parent = src;
		mapnods[nIdx][src.x][src.y].checked = true;
		...
		while (!openList.empty())
		{
			...
			if ( mapnods[nIdx][curPos.x][curPos.y].deapth >= deapth )
			{
				continue;
			}
			...
			for (int32_t i = DOWN; i < DIRECTION_NUM; ++i)
			{
				...
				if ( !m_map.isWalkablePosition( pos.x, pos.y ) )
				{
					continue;
				}

				if ( mapnods[nIdx][pos.x][pos.y].checked )
				{
					continue;
				}

				mapnods[nIdx][pos.x][pos.y].checked = true;
				mapnods[nIdx][pos.x][pos.y].parent = curPos;
				mapnods[nIdx][pos.x][pos.y].deapth = mapnods[nIdx][curPos.x][curPos.y].deapth+1;
		...
		if ( bFind )
		{
			...
			FindNode* pNode = &mapnods[nIdx][curPos.x][curPos.y];
			while ( pNode->parent != src )
			{
				path.push_back( curPos );
				curPos = pNode->parent;
				pNode = &mapnods[nIdx][curPos.x][curPos.y];
			}
...
PositionVector PathFinder::findPathOctree(Position src, Position dest)
{
	...
	if (pTiles != NULL && pSrc != NULL && pDest != NULL && pDest->isWalkable() && !m_map.inStack( dest.x, dest.y ) )
	{
		...
		while (start.x != end.x || start.y != end.y)
		{
			...
			if (pStart == NULL || !pStart->isWalkable() || pEnd == NULL || !pEnd->isWalkable() || m_map.inStack( start.x, start.y ) || m_map.inStack( end.x, end.y ) )
			{
				break;
			}
