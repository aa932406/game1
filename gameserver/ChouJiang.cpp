...
#define MAX_LUCK_POINT	500 //�������ֵ
#define CHOU_JIANG_RECORD_MAX_COUT	50 //�齱��¼���50��
...
int32_t	ChouJiang::OnAskChouJiang( Answer::NetPacket *inPacket )
{
	...
	if ( BindTimes < ChouJiangTimes && ( AllTimes - BindTimes ) < ChouJiangTimes )//����Ʒ,�ǰ���Ʒ,���ܻ��ʹ��
	{
...
int32_t ChouJiang::OnTidy( Answer::NetPacket *inPacket )
{
	...
	for ( ; it != ItemTmp.end(); ++it )
	{
		for (  int32_t i = 0; i < MAX_ITEM_LIST; i++  )
		{
			...
			{
				if ( m_ItemList[i].itemClass == IC_EQUIP || m_ItemList[i].itemClass == IC_PET_EGG ) // װ�����ܵ���
				{
					continue;
				}
