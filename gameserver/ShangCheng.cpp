...
int32_t CShangCheng::BuyShopItem( int32_t ShopId, int32_t ItemCount, bool AddBag )
{
	...
	if ( IsLimited( pShangCheng, ItemCount ) )	//�жϸ�����ʱ������
	{
		return ERR_SYETEM_ERR;
	}
	...
	if ( AddBag )
	{
		...
		{
			LOG_ERROR("�����̳���Ʒʧ��,itemClass=%d,ItemId=%d,ItemCount=%d,time=%d,cid=%lld",pShangCheng->ItemClass,pShangCheng->ItemId,ItemCount,m_pPlayer->getNow(),m_pPlayer->getCid() );
		}
