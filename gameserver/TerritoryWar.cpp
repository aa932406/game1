...
#define MAX_REVIVE_TIMES	3		// 复活次数
...
void CTerritoryWar::SendPlayerActivityInfo( Player* player )
{
	...
	packet->writeInt32( memberCount );					// 参与人数
	packet->writeInt32( getActivityTime() );			// 时间
...
{
	int32_t startDays = TIMER.GetDaysFromStart();	// 开服、合服后第二天双倍奖励
	return startDays <= 1;
}
