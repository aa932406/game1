...
namespace Answer
{
	#define CLIENT_TO_LOGIN_KEY      "2E69D46F29FACD2FB8CA1D6FBDBD4834!"
	enum PackType
	{
	...
	public:
		...
		void decryCode()
		{
			const char* pKey = {CLIENT_TO_LOGIN_KEY};
			int32_t uBeginPlace=3;
			int32_t uLen =m_wOffset;
			int8_t KeyLen = (int8_t)strlen(pKey);
			for (int32_t i = 0; i < uLen; i++)
			{
				m_buffer[i] ^= pKey[(i+uBeginPlace)%KeyLen];
			}
		}

		bool decryptAndCheck()
		{
			decryCode();
			m_rOffset = m_wOffset - 5;
			m_autoIncrease = readInt32();
			if(m_autoIncrease <0) return false;
			char checkByte = 0;
			uint32_t iByte = 4;
			while (iByte < m_wOffset)
			{
				checkByte ^= m_buffer[iByte];
				++iByte;
			}
			bool result = (checkByte == 0);
			m_rOffset = 0;
			m_wOffset -= 5;
			return result;
		}

		int32_t getAutoIncrease()
		{
			return m_autoIncrease;
		}
