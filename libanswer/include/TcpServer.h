...
namespace Answer
{
	...
	{
	public:
		TcpServer(unsigned int netpackSize, unsigned int initNetpacketCount, unsigned int maxNetpacketCount, bool bCompress, bool log = false);
		virtual	~TcpServer();
	...
	public:
		...
		void replySuccess(Answer::TcpConnection *conn, uint16_t proc, int64_t addon = 0);
		void replyFailure(Answer::TcpConnection *conn, uint16_t proc, int32_t errcode, int64_t addon = 0);
