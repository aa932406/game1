...
: m_fd(INVALID_SOCKET), m_processor(NULL), m_packet(NULL), m_nextPacket( NULL )
{
...
{
	return m_sendLen == 0 && ( m_sendList.size() > 0 || m_nextPacket != NULL ) ;
}
...
void TcpConnection::decodeRecvBuffer()
{
	...
	//再次判断是否有qq数据包
#ifdef LIBANSWER_WINDOWS
		if (_strnicmp("tgw_", m_recvBuffer + decodeOffset, 4) == 0)
#endif
#ifdef LIBANSWER_LINUX
			if (strncasecmp("tgw_", m_recvBuffer + decodeOffset, 4)==0)
#endif
			{
				char *pChar = m_recvBuffer + decodeOffset;
				while (pChar < m_recvBuffer + m_recvBufferSize - 4)
				{
#ifdef LIBANSWER_WINDOWS
					if (_strnicmp("\r\n\r\n", pChar, 4) == 0)
#endif
...
#endif
						{
							decodeOffset = pChar + 4 - m_recvBuffer;//补全60位
							//decodeOffset += 20;
							isTgw = true;
	...
	if (m_bCheckPolicy && !isTgw)
	{
		if (m_recvOffset >= 23)
		{
#ifdef LIBANSWER_WINDOWS
			if (_strnicmp("<policy-file-request/>", m_recvBuffer, 22) == 0)
#endif
#ifdef LIBANSWER_LINUX
...
void TcpConnection::encodeSendBuffer()
{
	...
	if ( NULL == m_nextPacket )
	{
		m_nextPacket = m_sendList.pop();
	}
	while (m_nextPacket != NULL)
	{
		int len = (std::min)(static_cast<int>(m_nextPacket->getSize())-m_parseLen, m_sendBufferSize-4-m_sendLen);
		if (len <= 0)
		{
			LOG_ERROR("TcpConnection::encodeSendBuffer len = %d, nextPacket->getSize() = %d, m_parseLen = %d, m_sendBufferSize = %d, m_sendLen = %d\n", len, m_nextPacket->getSize(), m_parseLen, m_sendBufferSize, m_sendLen);
			return;
		}

		memcpy(m_sendBuffer+m_sendLen, m_nextPacket->getBuffer()-NET_PACKET_HEAD_LEN+m_parseLen, len);
		if (m_nextPacket->isPolicy())
		{
			bPolicy = true;
		}

		m_parseLen += len;
		if (m_parseLen == static_cast<int>(m_nextPacket->getSize()))
		{
			m_sendListByteLen -= m_nextPacket->getSize();
			m_nextPacket->destroy();
			m_nextPacket = m_sendList.pop();
			m_parseLen = 0;
		}
