...
int64_t MySqlQuery::getInt64Value( const char *fieldName, const int intNull/* =0 */ )
{
	if (m_result == NULL || m_currentRow == NULL || fieldName == NULL)
	{
		return intNull;
	...
	if (m_result == NULL || m_currentRow == NULL || m_fieldCount <= 0 || fieldIndx < 0 || fieldIndx >= m_fieldCount || m_currentRow[fieldIndx] == NULL)
	{
		return intNull;
	}

	return _atoi64(m_currentRow[fieldIndx]);
...
const char* MySqlQuery::getOneCell(const char *strNull/* = */)
{
...
int MySqlDB::open(const std::string &host, const std::string &user, const std::string &passwd, const std::string &db, unsigned int port)
{
	...
	if (mysql_real_connect(pConnection, host.c_str(), user.c_str(), passwd.c_str(), db.c_str(), port, NULL, CLIENT_MULTI_STATEMENTS) == NULL)
	{
