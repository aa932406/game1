...
bool TcpClient::connect(const InetAddress &serverAddr)
{
	...
	{
		LOG_ERROR("TcpClient::connect send return %d\n", err);
		Answer::close(m_fd);
		m_fd = INVALID_SOCKET;
		return false;
	}
	
	/*char szPolicy[] = "tgw_forplatformopen.qq.com\r\n\r\n";
	err = ::send(m_fd, szPolicy, sizeof(szPolicy)-1, 0);
	if (err != sizeof(szPolicy)-1)
	{
		LOG_ERROR("TcpClient::connect send return %d\n", err);
		Answer::close(m_fd);
		m_fd = INVALID_SOCKET;
...
NetPacket* TcpClient::popNetpacket()
{
	return popNetpacket(m_bufferSize);
}

NetPacket* TcpClient::popNetpacket(unsigned int buffersize)
{
