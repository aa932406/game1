...
int	CDBCFile::_ConvertStringToVector(
...
template < CDBCFile::FIELD_TYPE T>
bool CDBCFile::_FieldEqu(const FIELD& a, const FIELD& b)
{
	if(T == T_INT)
	{
...
CDBCFile::CDBCFile(unsigned int id)
{
	m_ID			= id;
	m_pStringBuf	= NULL;
	m_nIndexColum	= -1;
...
//从内存中字符串读取一行文本(按照换行符)
const char* CDBCFile::_GetLineFromMemory(
	char* pStringBuf, 
	int nBufSize, 
	const char* pMemory, 
	const char* pDeadEnd)
...
bool CDBCFile::OpenFromMemoryImpl_Binary(
	const char* pMemory, 
	const char* pDeadEnd, 
	const char* szFileName)
{
	...
	//------------------------------------------------------
	// Read Field Types
	vector< unsigned int > vFieldType;
...
bool CDBCFile::OpenFromMemoryImpl_Text(
	const char* pMemory, 
	const char* pDeadEnd,
	const char* szFileName)
{
...
const CDBCFile::FIELD* CDBCFile::Search_Index_EQU(int iIndex) const
{
...
const CDBCFile::FIELD* CDBCFile::Search_Posistion(int nRecordLine, int nColumNum) const
{
	...
	{
		char szTemp[260];
		//_snprintf(szTemp, MAX_FILE_PATH, "DBC:Invalid search request[rec=%d, col=%d]", nRecordLine, nColumNum);
...
//查找某列等于指定值的第一行
const CDBCFile::FIELD* CDBCFile::Search_First_Column_Equ(int nColumnNum, const FIELD& value) const
{
