...
TcpAcceptor::TcpAcceptor(TcpServer *server, unsigned int initConnCount, unsigned int maxConnCount, bool bCompress)
	: m_server(server), m_bCompress(bCompress), m_listening(false), m_listenFd(INVALID_SOCKET), m_listenPort(0), m_connPool(initConnCount, maxConnCount)
{
...
void TcpAcceptor::run()
{
	...
	while (m_listening)
	{
		...
		{
			LOG_DEBUG("TcpAcceptor::run, accept error, return INVALID_SOCKET, errno=%d\n", WSAGetLastError());
		}
		else
		{
...
int TcpAcceptor::startListen(int port, int backlog, int iothreadnum, int processthreadnum, unsigned int packSize, unsigned int initPackCount, unsigned int maxPackCount)
{
	...
	do 
	{
		...
		LOG_INFO("try to bind to port %d\n", port);
