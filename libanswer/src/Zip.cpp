...
int32_t Zip::uncompressFile(const char *filename, uint8_t *dest, uint32_t *destLen)
{
	...
	if (zerr != Z_OK)
	{
		LOG_ERROR("Zlib::uncompressFile uncompress return %d when uncompress %s\n", zerr,filename);
...
int32_t Zip::uncompressbuf(char *dest, unsigned long destlen, const char *src, unsigned long srclen)
{
	int zerr = uncompress((Bytef*)dest, &destlen, (const Bytef*)src, srclen);
	if (zerr != Z_OK)
	{
		LOG_ERROR("Zip::uncompressbuf return %d\n", zerr);
		return -1;
	}
